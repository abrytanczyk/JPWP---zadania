    ...
    public String information(String phrase){
        ...
        try{
            for (String url: addresses) {
                ...
                //Find all <a> tags
                // 1.1 Znajdź w documencie wszystkie znaczniki <a ...>, zapisz je jako allA
                ...
                for (Element elem : allA) {
                    ...
                    if (elem.text().toLowerCase().contains(phrase.toLowerCase())) { // 1.1 Znajdź w tekście wszystkie wystąpienia phrase (przydatna może okazac się funkcja text())
                        //Handle relative addresses
                        if (elem.attr("href").startsWith("/")) {
                            //1.1 Utwórz hiperłącze na wyszukanym tekście, który umieścisz jako h2. Warto opakować hiperłącza w containery
    ...
    public String newsToNewsletter(){
        ...
        try{
            ...
            for (String url: addresses) {
                ...
                Document document = connect.get();
                //Find all <a> tags
                // 1.1 Znajdź w documencie wszystkie znaczniki <a ...>, zapisz je jako allA - done
                Elements allA = document.select("a");
                //Search through all found <a> tags
                for (Element elem : allA) {
                    //Compare given phrase with text from Elements
